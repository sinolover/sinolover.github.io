lambda 表达式是 [C++](http://c.biancheng.net/cplus/)11 最重要也最常用的一个特性之一，[C#](http://c.biancheng.net/csharp/) 3.5 和 [Java](http://c.biancheng.net/java/) 8 中就引入了 lambda 表达式。  
...
就地定义匿名函数，不再需要定义函数对象，大大简化了标准库算法的调用。比如，在 C++11 之前，我们要调用 for\_each 函数将 vector 中的偶数打印出来，如下所示。  
